//SERWER TYPE 1.1 - "Temperaturowe" emulowane Arduino
...
#define LOCAL_PORT                49001  //port tego serwera
...
#define SERVER_T1_ID              '\x10' //0001 0000 (numer 1 dla 3 bitów ID)
...
void sendTEMP_RESP() {
  ...
  uint8_t header = SERVER_TYPE_1 | SERVER_T1_ID | TEMP_RESPONSE;  //naglowek protokolu - 1 (TYPE 1), 001 (SERVER ID), 0101 (TEMP_RESPONSE)
...
void listenUDP() {
  ...
  if(packetSize>0) {
    ...
    //Weryfikacja, czy wiadomosc jest skierowana do tego serwera (ID: 001) i jego typu (type 1: 1)
